...
class WeatherControllerTest extends GetxController {
  var weatherData = {}.obs;
  GetStorage box = GetStorage();
  final WeatherService weatherService;
  WeatherControllerTest({required this.weatherService});
  void fetchWeather(String city) async {
    try {
      var data = await weatherService.getWeather(city);
      weatherData.value = data;
      box.write("city", data['location']['name']);
    } catch (e) {
      print(e);
    }
  }
/* 
  ...
  Future fetchData(String lat, String lon, String city) async {
    locationList = <Location>[].obs;
    var baseWeather =
        "$base/forecast.json?lang=ar&key=$api_key&q=$city&days=7&lat=$lat&lon=$lon";
    try {
      isLoading.value = true;
      printInfo(info: " {page=> of weather}");
      Uri uri = Uri.parse(baseWeather);
